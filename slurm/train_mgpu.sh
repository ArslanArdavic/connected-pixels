...
# Request 4 GPUs by default for multi-GPU DDP
# (Override with sbatch -G <n> if needed)
#SBATCH --gpus=4
...
# Launch multi-GPU training (one process per GPU) with default args
NUM_GPUS="${SLURM_GPUS:-4}"

echo "Launching train_parallel.py on ${NUM_GPUS} GPUs"
