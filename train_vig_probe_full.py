...
from all_data_masked import build_all_masked_loaders
...
# COCO 2017 root (installed under <submit_dir>/data/coco2017)
COCO_ROOT = os.path.join(os.environ.get("SLURM_SUBMIT_DIR", os.getcwd()), "data", "coco2017")
...
def main():
    ...
    if USE_NEPTUNE and neptune is not None and NEPTUNE_API_TOKEN:
        tags = ["vig_ti", "mpp", "linear_probe", f"mpp_lr_{PRETRAIN_LR}", f"probe_lr_{PROBE_LR}", "COCO + ImageNet"]
        run = neptune.init_run(
            project=NEPTUNE_PROJECT,
            api_token=NEPTUNE_API_TOKEN,
            name="vig_ti COCO + ImageNet mpp + linear_probe",
            tags=tags,
        )
        run["config"] = {
    ...
    pretrain_train_loader, pretrain_val_loader, _wnid, combined_train_dataset = build_all_masked_loaders(
    imagenet_train_root=TRAIN_ROOT,
    imagenet_devkit_root=DEVKIT_ROOT,
    imagenet_val_fraction=VAL_FRACTION,
    coco_root=COCO_ROOT,
    ...
    for epoch in range(1, PRETRAIN_EPOCHS + 1):
        # reshuffle interleaving order each epoch while keeping even COCO spread
        combined_train_dataset.set_epoch(epoch)
